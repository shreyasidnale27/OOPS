// --------------------------------------------------------------------------------------------------------------------
// <copyright file="AllFilePaths.cs" company="Bridgelabz">
//   Copyright © 2019 Company="BridgeLabz"
// </copyright>
// <creator name="Shreya Sidnale"/>
// --------------------------------------------------------------------------------------------------------------------
namespace Object_Orienetd_Programming
{
    using System;
    using System.Collections.Generic;
    using System.Text;

    /// <summary>
    /// class for paths for all Java Script Object Notation file 
    /// </summary>
    public class AllFilePaths
    {
        /// <summary>
        /// The path for the Java Script Object Notation file products and stocks
        /// </summary>
        public readonly string InventoryItems = @"C:\Users\admin\source\repos\Object Orienetd Programming\Object Orienetd Programming\InventoryDetails\GroceryInventory.json";
        public readonly string StockData = @"C:\Users\admin\source\repos\Object Orienetd Programming\Object Orienetd Programming\StockReport\StockData.json";
    }
}
